...
type JWTInterface interface {
	GenerateJWT(userID uint, nama string) string
	GenerateToken(id uint, nama string) string
...
func (j *JWT) GenerateJWT(userID uint, nama string) string {
	var accessToken = j.GenerateToken(userID, nama)
	if accessToken == "" {
		return ""
	}
	return accessToken
...
func (j *JWT) GenerateToken(id uint, nama string) string {
